...
\begin{align*}
\min_x \frac{1}{2} x^\top Q x + x^\top f, \\
\text{subject to: } \ell \leq A x \leq u
\end{align*}

By introducing auxiliary variables, this can be converted into a simple
box-constrained problem. First, introduce the variable $y$, with the constraint
that $A x = y$:

\begin{align*}
\min_x \frac{1}{2} x^\top Q x + x^\top f, \\
\text{subject to: } A x = y \quad \text{ and } \quad
\ell \leq y \leq u
\end{align*}

To remove the [linear equality constraints](#4-linear-equality-constraints) on
$x$ and $y$, let use consider the [singular value
decomposition](https://en.wikipedia.org/wiki/Singular_value_decomposition) of
$A \in \mathbb{R}^{m \times n}$:

$$ U S V^\top = A$$

where $U \in \mathbb{R}^{m \times m}$ is a unitary matrix, $S \in \mathbb{R}^{m
\times n}$ is a sparse matrix with non-increasing, non-negative entries only along the main
diagonal, and $V \in \mathbb{R}^{n \times n}$ is a unitary matrix. Consider the
largest index $k$ such that $S(k,k) \ge \varepsilon$. We declare $k$ to be the
numerical rank of $A$. We can make two observations. If we set 

\begin{align*}
N = V \begin{bmatrix} 0 \\ I_{n-k} \end{bmatrix} \in \mathbb{R}^{n \times (m-k)}
\quad 
\text{and}
\quad 
A^+ = V \begin{bmatrix}
(S_{1\dots k,1\dots k})^{-1} & 0\\
0 & 0
\end{bmatrix} U^\top
...
then $A N z = 0$ for any choice of
$z\in \mathbb{R}^{m-k}$. Meanwhile, $A A^+ = I$, and we call $A^+$ a
pseudo-inverse of $A$.  From these two definitions, if we substitute $x = A^+ y
+ N z$, then

$$
A \underbrace{\begin{bmatrix} A^+ & N \end{bmatrix}}_{M \in \mathbb{R}^{n \times (m+k)}} \underbrace{\begin{bmatrix} y \\ z
\end{bmatrix}}_{w \in \mathbb{R}^{m-k}}  = y \longrightarrow y = y 
$$

Substituting $x = M w$ into the problem above we see that only box constraints
remain:

\begin{align*}
\min_{w} \frac{1}{2} w^\top M^\top  Q M w + w^\top M^\top f, \\
\text{subject to: } 
\begin{bmatrix}\ell\\ -\infty \end{bmatrix} \leq w \leq  \begin{bmatrix} u\\ \infty \end{bmatrix}
\end{align*}
