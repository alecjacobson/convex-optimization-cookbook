...
> approach is to build for the index set $I$ a sparse selection matrix $S^I \in
> \mathbb{R}^{k \times n}$ such that:
>
> $$S^I_{ij} = 1 \iff I_i = j$$
>
> This way $x_I = S^I x$. Follow the same construction for 
> $S^U \in \mathbb{R}^{n-k \times n}$, _mutatis mutandis_. Then you have 
> $Q_UI = (S^U)^\top Q (S_I)$ and so on.
